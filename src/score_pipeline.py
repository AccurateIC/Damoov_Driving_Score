...
def run_score_pipeline(db_path, config):
    ...
    for uid in main_df['unique_id'].unique():
        ...
        print(trip_df[['speed_kmh']].describe())
    ...
    updated_df = pd.merge(main_df, score_df, on='unique_id', how='left')
    updated_df.to_sql("SampleTable", conn, if_exists="replace", index=False)
    print(main_df['timestamp'].head(10))
    conn.close()
...
def run_score_pipeline(db_path, config):
    ...
    # ✅ Fix: Convert tick_timestamp to real datetime
    print("🔍 Converting tick_timestamp to datetime...")
    ...
    # Confirm range
    print("📅 Timestamp range:", main_df['timestamp'].min(), "→", main_df['timestamp'].max())
    ...
    for uid in main_df['unique_id'].unique():
        ...
        print(f"\n🚘 Scoring Trip UID: {uid} | Distance: {trip_distance:.2f} km | Rows: {len(trip_df)}")

        # Skip short trips
        if len(trip_df) < 10 or trip_distance <= 1.0:
            print("⏭ Skipping due to short trip.")
            continue
        ...
        acc.calculate_penalties()
        acc_score = acc.get_events()['penalty_acc'].sum() if not acc.get_events().empty else 0
        print(f"⚡ Acc Events: {len(acc.get_events())}")
    ...
    if score_df.empty:
        print("⚠️ No valid trips to score. Exiting.")
        conn.close()
        return
    ...
    # ✅ Show all rows in print (if needed)
