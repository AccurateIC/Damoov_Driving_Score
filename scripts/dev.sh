...
nohup node node_modules/vite/bin/vite.js --host $FRONTEND_HOST --port $FRONTEND_PORT > "$LOG_DIR/frontend.log" 2>&1 &
cd ..
...
nohup python3 -m src.flask_server --host $BACKEND_HOST --port $BACKEND_PORT > "$LOG_DIR/backend.log" 2>&1 &
deactivate
cd ..
